...
export async function fbPageOverview(): Promise<ApiResult<{
  ...
  const pageRes = await jfetch<{ id: string; name: string; fan_count?: number; followers_count?: number }>(
...
export async function igAccountOverview(): Promise<ApiResult<{
  ...
  if (igBiz && isFbGraphToken) {
    const accRes = await jfetch<{ id: string; username: string; followers_count?: number; media_count?: number }>(
    ...
    if (accRes.ok) {
      ...
      if (mediaRes.ok) return { ok: true, data: { account: accRes.data, recentMedia: mediaRes.data.data ?? [] } };
    }
  }
...
export async function waSendText(to: string, body: string): Promise<ApiResult<{ message_id: string }>> {
  const missing = envOk("WHATSAPP_ACCESS_TOKEN", "WHATSAPP_PHONE_NUMBER_ID");
  if (missing) return { ok: false, error: `missing_env:${missing}` };
  const tok = process.env.WHATSAPP_ACCESS_TOKEN!;
  const phoneId = process.env.WHATSAPP_PHONE_NUMBER_ID!;
  const res = await jfetch<{ messages: Array<{ id: string }> }>(
...
export async function waSendTemplate(to: string, templateName: string, languageCode = "en_US"): Promise<ApiResult<{ message_id: string }>> {
  const missing = envOk("WHATSAPP_ACCESS_TOKEN", "WHATSAPP_PHONE_NUMBER_ID");
  if (missing) return { ok: false, error: `missing_env:${missing}` };
  const tok = process.env.WHATSAPP_ACCESS_TOKEN!;
  const phoneId = process.env.WHATSAPP_PHONE_NUMBER_ID!;
...
export async function waListTemplates(): Promise<ApiResult<{ data: Array<{ name: string; status: string; category: string; language: string }> }>> {
  const missing = envOk("WHATSAPP_ACCESS_TOKEN", "WHATSAPP_BUSINESS_ACCOUNT_ID");
  if (missing) return { ok: false, error: `missing_env:${missing}` };
  const tok = process.env.WHATSAPP_ACCESS_TOKEN!;
  const waba = process.env.WHATSAPP_BUSINESS_ACCOUNT_ID!;
  return jfetch(`${GRAPH}/${waba}/message_templates?limit=50&access_token=${encodeURIComponent(tok)}`);
}

// ============ Research (Tavily / SerpAPI / Firecrawl) ============
export type ResearchHit = { title: string; url: string; snippet: string; source: "tavily" | "serpapi" };

export async function researchSearch(query: string, maxResults = 5): Promise<ApiResult<ResearchHit[]>> {
    ...
    if (res.ok) {
      return {
        ...
        data: res.data.results.map((r) => ({
          title: r.title, url: r.url, snippet: (r.content ?? "").slice(0, 400), source: "tavily",
        })),
      };
    }
  }
  if (process.env.SERPAPI_KEY) {
    const res = await jfetch<{ organic_results?: Array<{ title: string; link: string; snippet?: string }> }>(
...
export async function firecrawlScrape(url: string): Promise<ApiResult<{ markdown: string; title?: string }>> {
  ...
  return {
    ok: true,
    data: { markdown: (res.data.data?.markdown ?? "").slice(0, 8000), title: res.data.data?.metadata?.title },
  };
}
...
export const INTEGRATION_DEFS: IntegrationDef[] = [
  { key: "meta_facebook", label: "Meta — Facebook Page", requiredEnv: ["META_APP_ID", "FACEBOOK_PAGE_ID", "FACEBOOK_PAGE_ACCESS_TOKEN"] },
  { key: "meta_instagram", label: "Meta — Instagram", requiredEnv: ["INSTAGRAM_ACCESS_TOKEN"], optionalEnv: ["INSTAGRAM_APP_ID", "INSTAGRAM_APP_SECRET"] },
  { key: "whatsapp", label: "WhatsApp Business Cloud", requiredEnv: ["WHATSAPP_ACCESS_TOKEN", "WHATSAPP_PHONE_NUMBER_ID", "WHATSAPP_BUSINESS_ACCOUNT_ID"] },
  { key: "google_base", label: "Google — OAuth Client + API Key", requiredEnv: ["GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "GOOGLE_API_KEY"] },
  { key: "google_ads", label: "Google Ads (pending)", requiredEnv: ["GOOGLE_ADS_DEVELOPER_TOKEN", "GOOGLE_ADS_REFRESH_TOKEN", "GOOGLE_ADS_MCC_ID"] },
  { key: "google_analytics", label: "Google Analytics 4 (pending)", requiredEnv: ["GA4_PROPERTY_ID", "GOOGLE_SERVICE_ACCOUNT_JSON"] },
  { key: "google_search_console", label: "Google Search Console (pending)", requiredEnv: ["GSC_SITE_URL", "GOOGLE_SERVICE_ACCOUNT_JSON"] },
  { key: "google_business_profile", label: "Google Business Profile (pending)", requiredEnv: ["GBP_ACCOUNT_ID", "GBP_LOCATION_ID"] },
  { key: "gmail", label: "Gmail (pending refresh token)", requiredEnv: ["GMAIL_REFRESH_TOKEN"] },
  { key: "firebase", label: "Firebase Admin", requiredEnv: ["FIREBASE_SERVICE_ACCOUNT_JSON", "FIREBASE_PROJECT_ID"] },
  { key: "cloudflare", label: "Cloudflare", requiredEnv: ["CLOUDFLARE_API_TOKEN", "CLOUDFLARE_ACCOUNT_ID"] },
  { key: "github", label: "GitHub", requiredEnv: ["GITHUB_TOKEN"] },
  { key: "twilio", label: "Twilio", requiredEnv: ["TWILIO_ACCOUNT_SID", "TWILIO_AUTH_TOKEN", "TWILIO_PHONE_NUMBER"] },
